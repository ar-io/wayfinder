...
async function afterPopupDOMLoaded(): Promise<void> {
	...
	const staticGatewaySetting = document.getElementById(
		'staticGatewaySetting',
	) as HTMLElement | null;
	...
	if (
		...
		saveStaticGatewayButton.addEventListener('click', async function () {
			const gatewayValue = (
		...
		saveArIOProcessIdButton.addEventListener('click', async function () {
			...
			if (arIOProcessId === '') {
				...
				saveArIOProcessId(arIOProcessId);
				return new Promise((resolve, reject) => {
					chrome.runtime.sendMessage(
						{ message: 'setArIOProcessId' },
						function (response) {
...
function saveRoutingMethod(inputValue: any) {
	chrome.runtime.sendMessage({ message: 'setRoutingStrategy', strategy: inputValue });
